...
fn base_config(tmp: &TempDir, port: u16, is_replica: bool) -> Config {
    let base = tmp
        .path()
        .join(if is_replica { "replica" } else { "primary" });
...
fn test_shipping_abuse_corrupted_segment() {
    ...
    .unwrap()
    .with_shipping(primary_cfg.shipping.ship_dir.clone(), ShipMode::Copy)
    ...
    assert!(
        res.is_err(),
        "VULNERABILITY SURFACED: Replica accepted corrupted segment!"
    );
...
fn test_shipping_abuse_out_of_order_manifest() {
    ...
    assert!(
        res.is_err(),
        "VULNERABILITY SURFACED: Replica accepted out-of-order manifest!"
    );
    let err_str = res.unwrap_err().to_string();
    assert!(
        err_str.contains("out of order") || err_str.contains("expected seq"),
        "Expected sequence error, got: {}",
        err_str
    );
}
