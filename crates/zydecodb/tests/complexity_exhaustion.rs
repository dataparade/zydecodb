...
fn test_filter_complexity_exhaustion() {
    ...
    write_request(&mut stream, &RequestEnvelope::new(Command::IndexDef, idx.encode()));
    ...
    write_request(&mut stream, &RequestEnvelope::new(Command::DocPut, put.encode()));
    assert_eq!(read_response(&mut stream).status, Status::Ok);
    ...
    assert!(
        ...
        "VULNERABILITY SURFACED: Deeply nested filter pinned the thread for {:?}", elapsed
    );
    ...
    if res.is_ok() {
        ...
        assert!(status == Status::ProtocolError || status == Status::Ok || status == Status::Error || status == Status::InvalidValue, "Unexpected status: {:?}", status);
    } else {
