...
use zydecodb::security::keys::{KeyRecord, TenantRecord};

fn evasion_config(tmp: &TempDir, listen: std::net::SocketAddr) -> zydecodb::config::Config {
    ...
    let keys_toml = format!(
        ...
        key_record.secret_lookup.unwrap(),
        key_record.tenant,
...
fn test_rate_limit_evasion_connection_cycling() {
    ...
    let addr = free_addr();
    let config = evasion_config(&tmp, addr);
        ...
        threads.push(thread::spawn(move || {
            ...
            while start_time.elapsed() < test_duration {
                if let Ok(mut stream) = TcpStream::connect(addr) {
                    ...
                    // Authenticate
                    let auth_req =
                        RequestEnvelope::new(Command::SessionInit, b"test_key:dummy".to_vec());
