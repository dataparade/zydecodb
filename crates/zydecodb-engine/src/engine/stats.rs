...
impl Engine {
    ...
    /// Cheap gauges from already-in-memory integers. Safe on every write.
    ...
    /// Refresh expensive gauges (WAL readdir, SST walks, block-cache snapshot).
    /// Prefer the maintenance poll / topology-change paths; exposed for tests
    /// and rare operator sync.
    pub fn refresh_metrics(&self) {
        self.refresh_topology_gauges();
    }

    /// Expensive gauges: WAL readdir, SST walks, block-cache snapshot, per-level
    /// Prometheus labels. Call only on topology changes (freeze/flush/compaction/
    /// WAL rotation) or from the maintenance poll thread — not every put.
    pub(crate) fn refresh_topology_gauges(&self) {
        self.update_gauges();
